...
library(tidyverse)
library(broom)
climate <- read_csv("data/Climate_Filtered_Data.csv")
...
climate2 <- climate %>%
  filter(`Fossil fuel energy consumption (% of total)_15`!= "..") %>%
  filter(`Population density (people per sq. km of land area)_15` != "..") %>%
  mutate(fossil_fuel_15 = as.numeric(`Fossil fuel energy consumption (% of total)_15`),
         population_density_15 = as.numeric(`Population density (people per sq. km of land area)_15`))%>%
  select(fossil_fuel_15, population_density_15)%>%
  slice(2:16)
  
climate_model <- lm(fossil_fuel_15 ~ population_density_15, data = climate2)
tidy(climate_model)
...
climateData2 <- climateData %>% 
  filter(`Series Name` == "CO2 emissions (metric tons per capita)" | `Series Name` == "Net migration" ) 
  #filter("")
